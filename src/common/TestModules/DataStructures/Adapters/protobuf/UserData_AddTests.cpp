...
class UserDataAddTests : public testing::Test{
protected:
...
TEST_F(UserDataAddTests, ctor){
    all_false();
}

TEST_F(UserDataAddTests, setDataByPointer ){
    sut.acl();
    sut.address();
    sut.avatar();
    sut.email();
...
TEST_F(UserDataAddTests, setDataBySetAccesor ){
...
TEST_F(UserDataAddTests, clearData ){
    sut.assign_acl( new ProtobufAclAdapter() );
    sut.set_address("adr");
    sut.set_avatar("avatardata");
    sut.set_email("email123@ww.ww");
